...
if [ $? -ne 0 ]; then
    ...
    # Adding extra layer (meta-essa-mx6ul)
    echo "# Custom layer to add HAB & DM-Crypt features" >>conf/bblayers.conf
    echo "BBLAYERS += \" \${BSPDIR}/sources/meta-essa-mx6ul \"" >>conf/bblayers.conf

    # Adding personalized configuration
    cat ../sources/meta-essa-mx6ul/templates/local.conf.append >>conf/local.conf
