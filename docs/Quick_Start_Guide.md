...
Create directory for the ESSA BSP for the i.<d/>MX 6UL EVK board:
...
The following shows all images created in build process. The full image for flash the i.<d/>MX 6UL EVK board is `core-image-base-imx6ulevk.wic.bz2`

<p align="left">
    <img src="assets/yocto_out_dir.png" alt="yocto_out_dir" />
</p>

Set the i.<d/>MX 6UL EVKboard to serial download protocol (_SDP_) mode by configuring the boot configuration switches following **Table 1**.
...
**TABLE 1**: Boot mode selection switches on i.<d/>MX 6UL EVK (_[ref](https://media.digikey.com/pdf/Data%20Sheets/Freescale%20Semi/i.MX_6UL_EVK_QSG.pdf)_)
...
Power down the board and set the i.<d/>MX 6UL EVKboard to the SD card (_SD4_) boot mode following **Table 1**
...
Connect to the `i.MX 6UL EVK` board `J16 - Debug PORT 1` with a **USB-UART** cable.
Power up the i.<d/>MX 6UL EVKboard and log in as: `root`
...
Reboot the i.<d/>MX 6UL EVK board and log in as: root
